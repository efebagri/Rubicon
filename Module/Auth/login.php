...
if(isset($_POST['login'])){
    ...
    if($system->getSetting('login') == 0){
        if($user->getDataByEmail($_POST['email'],'role') == 'mod' || $user->getDataByEmail($_POST['email'],'role') == 'admin'){
        } else {
    ...
    if($user->getState($_POST['email']) == 'banned'){
        $error = 'Your account is locked, contact email support.';
    }
